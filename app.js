...
function showWeakness(percents) {
  ...
  if (allSame) {
    // All categories are equal — message depends on grade level
    if (weakPct >= 80) {
  ...
  // Only show the tip box when there's a specific category weakness
...
function getGoalAdvice(percents, overall, target, ptsNeeded) {
  ...
  // Check if a second category could also achieve the goal
  let altHTML = '';
  if (ranked.length > 1) {
    const second    = ranked[1];
